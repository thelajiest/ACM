...
int check(const pt &a, const pt &b, const Polygon &hull) {
    int n = hull.size();
...
void solve() {
    ...
    std::vector<std::bitset<N>> adx(n);
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            if (i == j) continue;
            if (check(a[i], a[j], b) == -1) {
                adx[i][j] = 1;
            }
        }
    }
    ...
    for (int i = 0; i < n; i++) {
        std::queue<int> q;
        for (int j = 0; j < n; j++) res[j] = 1, dis[j] = -1;
        for (int j = 0; j < n; j++)
            if (adx[i][j]) dis[j] = 1, q.push(j), res[j] = 0;
        while (!q.empty() && dis[i] == -1) {
            int x = q.front();
            std::bitset<N> B = adx[x] & res;
            q.pop();
            while (B.any()) {
                int u = B._Find_first();
                B[u] = 0;
        ...
        if (dis[i] != -1) ans = std::min(ans, dis[i]);
    }

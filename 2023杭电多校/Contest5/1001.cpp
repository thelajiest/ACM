...
inline T cross(const pt &x, const pt &y) { return x.x * y.y - x.y * y.x; }
inline T dot(const pt &x, const pt &y) { return x.x * y.x + x.y * y.y; }

inline T abs2(const pt &x) { return dot(x, x); }
...
inline int ccw(const Point &a, Point b, Point c) {
    ...
    if (abs2(b) - abs2(c) < -eps) return -2;  // "ONLINE_FRONT"
    return 0;                                 // "ON_SEGMENT"
}
...
bool intersect(const Segment &s, const Point &p) {
    return abs(p.cross(s.a, s.b)) < eps && dot(s.a - p, s.b - p) <= eps;
}
...
double disPointSegment(const Point &p, const Segment &s) {
    if (s.a == s.b) return sqrt(abs2(p - s.a));

    T d = abs2(s.b - s.a),
      t = std::min(d, std::max(T(), dot(p - s.a, s.b - s.a)));
    return (sqrt(abs2((p - s.a) * d - (s.b - s.a) * t))) / d;
}
}  // namespace Geometry
...
int main() {
    ...
    std::vector<Geometry::Point> t(m), s(n);
    ...
    for (int j = 0; j < n; j++) {
        ...
        double ans = 1E18;
        for (int i = 0; i + 1 < m; i++) {
            ans = std::min(ans, Geometry::disPointSegment(v, {t[i + 1], t[i]}));
        }

        std::stringstream ss;
        ss << std::fixed << std::setprecision(5) << ans << "\n";

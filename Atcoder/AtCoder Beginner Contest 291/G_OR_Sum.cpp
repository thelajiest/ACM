...
struct CooleyTukey {
    ...
    template <typename T,
              enable_if_t<is_integral<T>::value, nullptr_t> = nullptr>
...
int main() {
    ...
    std::vector<i64> conv(N);

    for (int p = 0; p < 5; p++) {
        std::vector<i64> A(2 * N), B(2 * N);
        for (int i = 0; i < N; i++) {
            A[i] = !((a[i] >> p) & 1);
            A[N + i] = A[i];
            B[N - i - 1] = !((b[i] >> p) & 1);
        }
        auto v = ArbitraryModConvolution::CooleyTukey::multiply(A, B);
        for (int i = 0; i < N; i++) {
            conv[i] += (N - v[N + i - 1]) << p;
        }
    }

    std::cout << *(std::max_element(conv.begin(), conv.end())) << std::endl;
    return 0;
}